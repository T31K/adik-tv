...
class ProfileRepository @Inject constructor(
     ...
     */
    // Megaflix: fixed profiles — no creation. Stable ids keep per-profile data working.
    private val hardcodedProfiles = listOf(
        Profile(id = "mom", name = "Mom"),
        Profile(id = "timon", name = "Timon"),
    )

    val profiles: Flow<List<Profile>> = kotlinx.coroutines.flow.flowOf(hardcodedProfiles)
    ...
    val activeProfile: Flow<Profile?> = context.profilesDataStore.data.map { prefs ->
        ...
        hardcodedProfiles.find { it.id == activeId } ?: hardcodedProfiles.first()
    }
